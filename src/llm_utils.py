...
def _get_available_model(api_key: str):
    ...
    preferred_models = ["models/gemini-2.5-pro", "models/gemini-pro-latest"]
    
    for model_name in preferred_models:
        try:
            model = genai.GenerativeModel(model_name)
            if model.count_tokens and model.generate_content: # Check for essential capabilities
                _cached_model = (model, model_name) # Cache the found model and its name
